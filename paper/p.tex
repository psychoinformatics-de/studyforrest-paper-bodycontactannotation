...
\subsection*{Observers} %find out age range

Seven observers (six female, age ??) independently annotated the movie. They
also developed the concept for this annotation.

\subsection*{Procedure} % I think they only annotated touching that was "in the main part of the picture" so nothing in the background since that would be a) tedious and b) super unreliable not sure though
			% also not sure about them writing down things like people being shown from the front but the posture indicates them touch eachother on their back (like the barscene with Dan) but I think they did
			% after clarification this has to be put in
...
\todo{invisible but implied touch?}
...
%this is just taken from the irony annotation with slight changes on the last part
%don't know about copy right stuff but since the procedure is basically identical maybe we can re-use it or just cite the irony paper.
